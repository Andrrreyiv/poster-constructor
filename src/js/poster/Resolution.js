...
/**
 * Мешает ли оценка оформить заказ.
 *
 * Сейчас не мешает никогда: клиент просил ПОКАЗЫВАТЬ сообщение, а не запрещать покупку,
 * и запрет без его слова был бы отсебятиной. Функция существует отдельно, чтобы запрет
 * включался одной строкой, если он передумает.
 */
export function blocksOrder() {
  return false;
}
